...
@routes.route('/event/<int:product_id>',methods=['GET'])
def get_product(product_id):
  product = next((event for event in products["event"] if event["id"] == product_id), None)
  if product:
    return jsonify(product)
  else:
    return jsonify({'Error':'Event tidak ada'}),404

@routes.route('/workshop/<int:product_id>',methods=['GET'])
def get_workshop(product_id):
  workshop = next((workshop for workshop in products["workshop"] if workshop["id"] == product_id), None)
  if workshop:
    return jsonify(workshop)
  else:
    return jsonify({'Error':'Workshop tidak ada'}),404
